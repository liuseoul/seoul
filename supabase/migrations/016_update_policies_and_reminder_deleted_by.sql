...
CREATE POLICY IF NOT EXISTS "已登录用户可更新工作记录" ON work_records
  FOR UPDATE USING (auth.role() = 'authenticated');

-- Allow any authenticated user to update time_logs (needed for soft-delete)
CREATE POLICY IF NOT EXISTS "已登录用户可更新工时记录" ON time_logs
  FOR UPDATE USING (auth.role() = 'authenticated');

-- Add deleted_by UUID to reminders so restore permission can be checked
